...
public class PomodoroReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (alarm_type == Pomodoro.ALARM_TYPE_TOMATO) {
			vibrate = prefs.getBoolean(Pomodoro.PREF_TOMATO_VIBRATE,
					Pomodoro.PREF_TOMATO_VIBRATE_DEFAULT);
			tone = prefs.getString(Pomodoro.PREF_TOMATO_RINGTONE,
					Pomodoro.PREF_TOMATO_RINGTONE_DEFAULT);
			if (tone != null)
				uri = Uri.parse(tone);
			volume = prefs.getInt(Pomodoro.PREF_TOMATO_VOLUME,
					Pomodoro.PREF_TOMATO_VOLUME_DEFAULT);
			delay = prefs.getInt(Pomodoro.PREF_TOMATO_DELAY,
					Pomodoro.PREF_TOMATO_DELAY_DEFAULT);
		} else if (alarm_type == Pomodoro.ALARM_TYPE_REST) {
			vibrate = prefs.getBoolean(Pomodoro.PREF_REST_VIBRATE,
					Pomodoro.PREF_REST_VIBRATE_DEFAULT);
			tone = prefs.getString(Pomodoro.PREF_REST_RINGTONE,
					Pomodoro.PREF_REST_RINGTONE_DEFAULT);
			if (tone != null)
				uri = Uri.parse(tone);
			volume = prefs.getInt(Pomodoro.PREF_REST_VOLUME,
					Pomodoro.PREF_REST_VOLUME_DEFAULT);
			delay = prefs.getInt(Pomodoro.PREF_REST_DELAY,
					Pomodoro.PREF_REST_DELAY_DEFAULT);
		}
